...
public class BeaconConfig {
    ...
    public static Path getLocalBeaconAuthFile() {
        ...
        if (OsType.ofLocal() == OsType.LINUX) {
            ...
            return Path.of(System.getProperty("java.io.tmpdir"), staging ? "bastion-ptb" : "bastion", name, "beacon-auth");
        } else {
            var path = Path.of(System.getProperty("java.io.tmpdir"), staging ? "bastion-ptb" : "bastion", "beacon-auth");
            if (path.startsWith(Path.of("C:\\Windows"))) {
                path = Path.of(System.getenv("LOCALAPPDATA"))
                        .resolve("Temp", staging ? "bastion-ptb" : "bastion", "beacon-auth");
            }
            return path;
        }
    }
